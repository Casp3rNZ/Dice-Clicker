...
public class ShopItem : ScriptableObject
{
    ...
    /// </summary>
    private static BigInteger CalculatePrice(BigInteger basePrice, float growthRate, int totalPurchased)
    {
        if (totalPurchased <= 0) return basePrice;
        return new BigInteger((double)basePrice * Math.Pow(growthRate, totalPurchased));
    }
    ...
    {
        return CalculatePrice(BigInteger.Parse(price), priceGrowthRate, totalPurchased);
    }
    ...
    {
        return CalculatePrice(new BigInteger(autoClickBasePrice), autoClickPriceGrowthRate, totalPurchased);
    }
}
